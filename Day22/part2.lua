...
local function calculateFall(block)
  local fall = 0
  local queue = {}
  local fallenBlocks = { [block] = true }

  if forwardDependencies[block] then
    for dependent, _ in pairs(forwardDependencies[block]) do
      table.insert(queue, dependent)
    end
  end

  while next(queue) do
    local shakingBlock = table.remove(queue, 1)
    local supportCount = 0
    for support, _ in pairs(backwardDependencies[shakingBlock]) do
      if not fallenBlocks[support] then
        supportCount = supportCount + 1
      end
    end

    if supportCount <= 0 then
      if not fallenBlocks[shakingBlock] then
        fall = fall + 1
        fallenBlocks[shakingBlock] = true

        if forwardDependencies[shakingBlock] then
          for dependent, _ in pairs(forwardDependencies[shakingBlock]) do
            table.insert(queue, dependent)
          end
        end
      end
...
for block, _ in ipairs(blocks) do
  result = result + calculateFall(block)
end
